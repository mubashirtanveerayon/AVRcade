...
static void spi_set_read_speed(void)
{
    /*
     * Mode 0, master, fCPU/16 = 500 kHz. Boot time is less important than
     * reliable reads while flash pages are being replaced.
     */
...
static void quiesce_oled(void)
{
    /*
     * The OLED controller is powered independently and is not reset by the
     * ATmega32 RESET pin. Force its hardware reset so a bright game frame is
     * not still active while the bootloader reads and programs from the SD.
     */
    PORTB |= (1U << OLED_CS) | (1U << SD_CS);
...
static uint8_t program_file_reliably(const char *filename)
{
    uint8_t result = program_file(filename);

    if (result == BOOT_READ_ERROR) {
        /*
         * A reset or transient can interrupt a streamed PetitFS read. Remount,
         * reopen, and restart from byte zero once. Pages already written are
         * verified and skipped when they already match.
         */
        if (mount_card() == 0U) {
            return BOOT_READ_ERROR;
        }
        result = program_file(filename);
    }
...
int main(void)
{
    ...
    /*
     * A mechanical RESET switch may bounce several times. Each bounce restarts
     * this delay, so no SD read or flash write begins until RESET is stable.
     * The pause also lets the continuously powered OLED and SD bus settle.
     */
    spi_init_slow();
    quiesce_oled();
    delay_100ms(2U);
    ...
    if (has_selection != 0U) {
        result = program_file_reliably(selected_name);
        if (result == BOOT_OK) {
    ...
    result = program_file_reliably(fallback_name);
    if (result != BOOT_OK) {
        fatal_blink(result);
    }
