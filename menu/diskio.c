...
/* Final handheld wiring: SD card CS is PB4. */
#define CS_LOW()	PORTB &= ~_BV(PB4)
#define	CS_HIGH()	PORTB |=  _BV(PB4)
#define	IS_CS_LOW	!(PINB & _BV(PB4))

/* We are not streaming to UART in this project, so FORWARD does nothing */
#define	FORWARD(d)	(void)(d)

/* Link to the hardware SPI function already written in main.c */
extern uint8_t spi_transfer(uint8_t data);

/* Replace Assembly functions (usi.S / suart.S) with standard C */
static void dly_100us (void) {
    _delay_us(100);
}

static void init_spi (void) {
    /* SPI and pin directions are already initialized in main.c */
}
